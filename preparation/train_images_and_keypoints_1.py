...
def path_generate_train_pairs(dest_dir):
    ...
    df.to_csv(os.path.join(dest_dir, "celeba-pairs-test.csv"), index=False, mode="w")
...
def path_generate_landmarks(dest_dir, landmark_root_dir, landmark_file):
    ...
    df = pd.read_csv(os.path.join(dest_dir, "celeba-pairs-test.csv"), dtype=str, sep=',')
...
def select_100_train():
    ...
    root_dir = "/home/tonyw/ba/anonygan/data/aligned"
    dest_dir = "/home/tonyw/ba/anonygan/output/BiGraphCeleba/train"
...
def select_100_test():
    # test set
    root_dir = "/home/tonyw/ba/anonygan/data/aligned"
    dest_dir = "/home/tonyw/ba/anonygan/output/BiGraphCeleba/test"
...
def extract_landmarks():
    ...
    dest_dir = "/home/tonyw/ba/anonygan/data"
    df = pd.read_csv(os.path.join(dest_dir, "celeba-pairs-train.csv"), dtype=str)
    all_values = df.to_numpy().flatten().tolist()
    all_values = list(map(lambda x: f"{x}.jpg", all_values))

    landmarks_df: pd.DataFrame = pd.read_csv("/home/tonyw/ba/anonygan/data/aligned/list_landmarks_align_celeba.txt", header=None, delim_whitespace=True)

    reduced_lndmks = landmarks_df[landmarks_df[0].isin(all_values)]

    root_dir = "/home/tonyw/ba/anonygan/data/img_align_celeba"
